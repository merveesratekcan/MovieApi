...
public class AdminCategoryController : Controller
{
    ...
    public async Task<IActionResult> CategoryList()
    {
        ...
        var responseMessage = await client.GetAsync("https://localhost:7288/api/Categories");
    ...
    public async Task<IActionResult> CreateCategory(AdminCreateCategoryDto AdminCreateCategoryDto)
    {
        ...
        var responseMessage = await client.PostAsync("https://localhost:7288/api/Categories", stringContent);
        if (responseMessage.IsSuccessStatusCode)
        {
